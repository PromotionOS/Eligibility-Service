...
public class CatalogItemExcludedConsumer implements MessageListener {
    ...
    public void onMessage(Message message, byte[] pattern) {
        try {
            CatalogItemExcludedEvent event = objectMapper.readValue(message.getBody(), CatalogItemExcludedEvent.class);
            eligibilityService.updateExclusions(event);
        } catch (NotImplementedException e) {
            log.debug("Skeleton stub: updateExclusions not yet implemented");
        } catch (Exception e) {
            log.error("Failed to process CatalogItemExcluded event", e);
        }
    }
}
